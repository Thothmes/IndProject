...
def report_2(f_check):
    ...
    if n == 0:
        ...
        for i in range(n):
            for j in range(0, n - i - 1):
                if volume_array[j] > volume_array[j + 1]:
                    year_rel_array[j], year_rel_array[j + 1] = year_rel_array[j + 1], year_rel_array[j]
                    volume_array[j], volume_array[j + 1] = volume_array[j + 1], volume_array[j]
                    surname_array[j], surname_array[j + 1] = surname_array[j + 1], surname_array[j]
        for i in range(n):
...
def report_3(f_check):
    ...
    try:
        ...
        for i in range(n):
            for j in range(0, n - i - 1):
                if year_brand_array[j][1] > year_brand_array[j + 1][1]:
                    year_brand_array[j], year_brand_array[j + 1] = year_brand_array[j + 1], year_brand_array[j]
        for i in range(n):
            for j in range(0, n - i - 1):
                if year_brand_array[j][0] < year_brand_array[j + 1][0]:
                    year_brand_array[j], year_brand_array[j + 1] = year_brand_array[j + 1], year_brand_array[j]
                ...
                try:
                    ...
                    try:
                        ...
                        if choise_n == 0:
                            ...
                            change_str = str(
                                input('Напишите все данные разделяя пробелами, согласно примеру и инструкции: '))
...
def check(data_str, f_check):
    ...
    if len(data_str) == 8:
        ...
        try:
            ...
            if not ((n_reg[0] in letters) and (n_reg[1] in numbers) and (n_reg[2] in numbers) and (
                    n_reg[3] in numbers) and (n_reg[4] in letters) and (n_reg[5] in letters)):
                if f_check == 1:
                    return 2
                elif f_check == 0:
